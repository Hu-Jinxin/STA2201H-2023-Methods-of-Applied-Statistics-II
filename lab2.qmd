...
library(janitor)
campaign_2014<-campaign2_2014
campaign_2014 |> group_by(contributors_name) |> summarize(sum_cont=sum(contribution_amount)) |> arrange(-sum_cont)|>slice(1:5)
campaign_2014 |> group_by(contributors_name) |> summarize(mean_cont=mean(contribution_amount)) |> arrange(-mean_cont)|>slice(1:5)
campaign_2014 |> mutate(name_count=str_count(contributors_name)) |> arrange(-name_count)|>slice(1:5)
```
...
library(janitor)
campaign_2014<-campaign2_2014
campaign_2014 |> group_by(contributors_name)|>filter(contributors_name!=candidate) |>summarize(sum_cont=sum(contribution_amount)) |> arrange(-sum_cont)|>slice(1:5)
campaign_2014 |> group_by(contributors_name)|>filter(contributors_name!=candidate) |> summarize(mean_cont=mean(contribution_amount)) |> arrange(-mean_cont)|>slice(1:5)
campaign_2014 |>filter(contributors_name!=candidate)|> mutate(name_count=str_count(contributors_name)) |> arrange(-name_count)|>slice(1:5)
```
...
camp<-campaign_2014 |> group_by(contributors_name)|>select(contributors_name,candidate) |> distinct() 
sum(table(camp$contributors_name)-1)
  
n<-length(camp$contributors_name)-length(unique(camp$contributors_name))
n
```

194 contributors gave money to more than one candidate.